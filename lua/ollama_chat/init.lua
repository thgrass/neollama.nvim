  ...
  if sess.added_buffers and #sess.added_buffers > 0 then
    ...
    for _, b in ipairs(sess.added_buffers) do
      if vim.api.nvim_buf_is_valid(b) then
        ...
        table.insert(pieces, string.format("<<FILE: %s>>\n%s", name ~= "" and name or ("[No Name " .. b .. "]"), table.concat(lines, "\n")))
      end
    end
...
-- HTTP call via curl using jobstart
local function http_generate(sess, prompt_text, on_done)
  ...
  local cmd = { "curl", "-sS", "-X", "POST", "-H", "Content-Type: application/json", url, "-d", payload }
  ...
  local job_id = vim.fn.jobstart(cmd, {
    ...
    on_exit = function(_, code, _)
      ...
      local resp = decoded and decoded.response or ""
      on_done(resp, nil)
    end,
  })
...
function M.add_current_buffer()
  ...
  append_lines(sess.buf, { ("_Added buffer to context:_ %s"):format(name ~= "" and name or ("[No Name " .. cur .. "]")), "" })
end
